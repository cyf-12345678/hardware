...
char ssid[33]="4#1305";          // wifi的SSID
char pwd[65]="jzf19741023";       // wifi的密码
char owner_id[64]="1";              // 用户ID

char open_broken_url[128]="127.0.0.1:18083";       // 二次开发mqtt服务器地址
char open_account[64]="admin";           // 二次开发mqtt账号
char open_pwd[64]="public";               // 二次开发mqtt密码
...
{
    ESP_LOGI(TAG, ".................. close led");
    gpio_set_level(IO_BUZZ, 1);
}

//初始化人体传感器
void sensor_human_init(void)
{
    gpio_reset_pin(SENSOR_HUMAN_PIN);
    gpio_set_direction(SENSOR_HUMAN_PIN, GPIO_MODE_INPUT);
}

//读取人体传感器
int sensor_human_get(void)
{
    return gpio_get_level(SENSOR_HUMAN_PIN);
}
